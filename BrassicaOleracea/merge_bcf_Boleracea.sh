#!/bin/bash
#PBS -q normal
#PBS -l select=1:ncpus=4:mem=192G
#PBS -l walltime=24:00:00
#PBS -N MergeBCFsBoleracea
#PBS -P Personal
...
# Define directories
BCF_DIR="/path/to/Boleracea/BCF_files"
OUTDIR="/path/to/Boleracea/VCF.GZ_file_output"
...
cat "$BCF_DIR/Boleracea_bcfs_merge.txt" | xargs bcftools concat -a --threads 4 \
    -o "$OUTDIR/Merged_BrassicaOleracea_BeforeFilter.vcf.gz" -O z

echo "####################"
echo "BCF MERGING COMPLETE"
echo "####################"

bcftools index "$OUTDIR/Merged_BrassicaOleracea_BeforeFilter.vcf.gz"