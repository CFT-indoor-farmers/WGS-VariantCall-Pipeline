...
# Define directories
FASTA_DIR="/path/to/fasta/file/BOleracea"
BAM_DIR="path/to/BOleracea/BAM_files"
OUTDIR="/path/to/BOleracea/BCF_file_output"
...
find "$BAM_DIR" -type f -name "*.bam" | sort | uniq > \
    "$BAM_DIR/BAMfie_BOleracea_list.txt"
...
bcftools mpileup --threads 4 -Ou -q 20 -Q 30 -f "$FASTA_DIR/BOleracea.fa" \
        -r "scaffold_$CHRNO" -b "$(echo $BAM_DIR/BAMfie_BOleracea_list.txt)" | \
        bcftools call --threads 4 -mv -Ob \
        -o "$OUTDIR/Boleracea_variants_scaffold$CHRNO.bcf"
